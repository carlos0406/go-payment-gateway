...
func (s *InvoiceService) ListByAccount(accountID string) (*domain.PaginationOutput[dto.InvoiceOutput], error) {
	pagination := domain.NewPagination(10, 1)
	result, err := s.invoiceRepository.FindByAccountID(accountID, pagination)
	if err != nil {
		return nil, err
	}

	data := make([]*dto.InvoiceOutput, len(result.Data))
	for i, invoice := range result.Data {
		data[i] = dto.FromInvoice(invoice)
	}
	output := &domain.PaginationOutput[dto.InvoiceOutput]{
		Data:        data,
		Total:       result.Total,
...
func (s *InvoiceService) ListByAccountAPIKey(apiKey string) (*domain.PaginationOutput[dto.InvoiceOutput], error) {
	accountOutput, err := s.accountService.FindByAPIKey(apiKey)
	if err != nil {
		return nil, err
	}
